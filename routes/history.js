...
router.get('/list/:page',isAlreadyAuth, function(req, res, next) {
    var perPage = 10
    var page = req.params.page || 1
    
    console.log('user session ==='+req.session.user);
    sessionvar.paginate({ $or : [{'userID':req.session.user},{'userID2':req.session.user}]}, { page: page, limit: perPage }).then(function(result) {
...
router.get('/updlist/:sess_id',isAlreadyAuth,function(req,res){
     req.session.session_id=req.params.sess_id;
     console.log("The session id opened is "+ req.session.session_id);
     res.redirect('/roles');
});
